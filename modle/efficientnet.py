...
class extra_CNN(nn.Module):
    def __init__(self, i):
        super(extra_CNN, self).__init__()
        self.layer1 = nn.Sequential(
            nn.UpsamplingBilinear2d(scale_factor=i)
        )
...
"""
    for i in range (len(output)):
        ...
        x = output[i].view(output[i].size(0), -1)
